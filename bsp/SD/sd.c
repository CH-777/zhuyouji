...
void SD_SPI_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(	RCC_APB2Periph_GPIOC, ENABLE ); //RCC_APB2Periph_GPIOA

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4;//GPIO_Pin_3;            //CD卡的片选引脚PA3，板子上为PC4
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP ;   
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &GPIO_InitStructure);
	GPIO_SetBits(GPIOC, GPIO_Pin_4);//GPIO_Pin_3);                    //PC4上拉，默认不选中，引脚拉低为选中 

	SPI1_Init();   //初始化SPI1
	SPI1_ReadWriteByte(0xff);//提高额外的8个时钟
	SD_CS = 1;     //拉高PC4
}
...
u8 SD_SendCmd(u8 cmd, u32 arg, u8 crc)
{
	...
	if(SD_Select())
		return 0XFF;//片选失效 
	//发送
    SD_SPI_ReadWriteByte(cmd | 0x40); //分别写入命令，命令前两位必须为01
    SD_SPI_ReadWriteByte(arg >> 24);  //MSB
	...
	}while((r1&0X80) && Retry--);	 //SD卡响应r1最高位始终为0，r1&0x80 == 0 表示已接收到响应 
...
/**
 * 获取SD卡的总扇区数（扇区数）   
 * 返回值:0： 取容量出错  其他:SD卡的容量(扇区数/512字节)
...
u8 SD_Initialize(void)
{
	...
	if(r1 == 0X01) {
		if(SD_SendCmd(CMD8,0x1AA,0x87) == 1) {         //SD V2.0
			...
			if(!retry || SD_SendCmd(CMD16, 512, 0X01) != 0)
...
u8 SD_ReadDisk(u8 *buf, u32 sector, u8 cnt)
{
	...
	if(cnt == 1) {
		...
		if(!r1)                              //指令发送成功
			r1 = SD_RecvData(buf, 512);      //接收数据起始令牌0xfe，然后接收512个字节	   
	}else {
...
u8 SD_WriteDisk(u8 *buf, u32 sector, u8 cnt)
{
	...
	if(cnt == 1) {
		...
		if(!r1)                              //指令发送成功
			r1 = SD_SendBlock(buf,0xFE);     //发送起始令牌0xfe，然后写512个字节	   
	}else {
		if(SD_Type != SD_TYPE_MMC) {
			SD_SendCmd(CMD55, 0, 0X01);	
			SD_SendCmd(CMD23, cnt, 0X01);    //发送擦除扇区指令	
		}
